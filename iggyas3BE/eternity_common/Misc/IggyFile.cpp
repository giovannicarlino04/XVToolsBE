...
// Constructor: initializes `is_big_endian` to false by default.
IggyFile::IggyFile(bool big_endian) : BaseFile(), is_64(false), is_big_endian(big_endian) {
...
bool IggyFile::LoadFlashData32(FixedMemoryStream &stream)
{
    ...
    // Swap header fields if the file is big-endian
    if (is_big_endian) {
        hdr.main_offset = __builtin_bswap32(hdr.main_offset);
        hdr.as3_section_offset = __builtin_bswap32(hdr.as3_section_offset);
        hdr.unk_offset = __builtin_bswap32(hdr.unk_offset);
        hdr.unk_offset2 = __builtin_bswap32(hdr.unk_offset2);
        hdr.unk_offset3 = __builtin_bswap32(hdr.unk_offset3);
        hdr.unk_offset4 = __builtin_bswap32(hdr.unk_offset4);
        hdr.width = __builtin_bswap32(hdr.width);
        hdr.height = __builtin_bswap32(hdr.height);
        hdr.names_offset = __builtin_bswap32(hdr.names_offset);
        hdr.last_section_offset = __builtin_bswap32(hdr.last_section_offset);
        hdr.as3_code_offset = __builtin_bswap32(hdr.as3_code_offset);
        hdr.as3_names_offset = __builtin_bswap32(hdr.as3_names_offset);
        hdr.unk_28 = __builtin_bswap32(hdr.unk_28);
        hdr.unk_58 = __builtin_bswap64(hdr.unk_58);
    }
    ...
    if (hdr.as3_section_offset >= size || hdr.unk_offset >= size || hdr.unk_offset2 >= size || hdr.unk_offset3 >= size) {
        DPRINTF("%s: Failure in some absolute offset. Offsets: as3_section_offset = 0x%X, unk_offset = 0x%X, unk_offset2 = 0x%X, unk_offset3 = 0x%X, size = 0x%X\n", 
                FUNCNAME, hdr.as3_section_offset, hdr.unk_offset, hdr.unk_offset2, hdr.unk_offset3, size);
        return false;
    }
    ...
    // Validate AS3 code section
    uint8_t *as3_ptr = as3_code_section.data();
    size_t max_as3_size = as3_code_section.size();
    if (is_big_endian) {
        // Swap AS3 section size if big-endian
        *(uint32_t *)&as3_ptr[4] = __builtin_bswap32(*(uint32_t *)&as3_ptr[4]);
    }
    if (max_as3_size != 0) {
...
bool IggyFile::Load(const uint8_t *buf, size_t size)
{
    ...
    if (hdr->plattform[0] != 1 || hdr->plattform[2] != 1)
    {
        ...
        //return false;
        DPRINTF("%s: Unknown plattform data, assuming bigEndian.\n", FUNCNAME);
    }
...
void IggyFile::SaveFlashData32(uint8_t *buf, size_t size)
{
    ...
    // Copy the sections into the buffer
    ...
    // Adjust offsets and byte-swap them
    hdr->as3_section_offset = Swap32((uint32_t)main_section.size());

    if (hdr->names_offset != Swap32(1)) {
        hdr->names_offset = Swap32(
            (uint32_t)(main_section.size() + as3_names_section.size() + as3_code_section.size() - offsetof(IGGYFlashHeader32, names_offset))
        );
    }

    if (hdr->last_section_offset != Swap32(1)) {
        hdr->last_section_offset = Swap32(
            (uint32_t)(size - last_section.size() - offsetof(IGGYFlashHeader32, last_section_offset))
        );
    }

    if (hdr->as3_code_offset != Swap32(1)) {
        hdr->as3_code_offset = Swap32(
            (uint32_t)(main_section.size() + as3_names_section.size() - offsetof(IGGYFlashHeader32, as3_code_offset))
        );
    }

    if (hdr->as3_names_offset != Swap32(1)) {
        hdr->as3_names_offset = Swap32(
            (uint32_t)(main_section.size() + 8 - offsetof(IGGYFlashHeader32, as3_names_offset))
        );
    }
}
...
uint8_t *IggyFile::Save(size_t *psize)
{
    ...
    // Fill in the header, applying byte-swapping where necessary
    hdr->signature = IGGY_SIGNATURE;
    hdr->version = 0x900;
    hdr->plattform[0] = hdr->plattform[2] = 2;
    hdr->plattform[1] = (is_64) ? 64 : 32;
    hdr->unk_0C = unk_0C;
    hdr->num_subfiles = 2;

    // Fill in the first subfile entry (Flash Data)
    entries[0].id = 1;
    ...
    offset += GetFlashDataSize();

    // Fill in the second subfile entry (Index Data)
    entries[1].id = 0;
    ...
    offset += (uint32_t)index_data.size();
...
bool IggyFile::SetAbcBlob(void *buf, uint32_t size)
{
    ...
    if (is_64)
    {
        ...
        *(uint32_t *)&ptr[8] = size;  // byte-swap size
    }
    else
    {
        memset(ptr+8, 0, new_size-8);
        memcpy(ptr+8, buf, size);
        *(uint32_t *)&ptr[4] = size;  // byte-swap size
    }

    if (hack)
    {
        if (is_64)
            *(uint32_t *)&ptr[new_size-0x10] = 1;  // byte-swap value
        else
            *(uint32_t *)&ptr[new_size-8] = 1;  // byte-swap value
    }
    ...
    while (stream.Read8(&code))
    {
        ...
        else if (code == 0xFF)
        {
            ...
            if (state == 2)
            {
                ...
                return stream.Write32(Swap32(n));  // byte-swap value
            }
